...
func (e *Executor) Execute(ctx context.Context, plan ExecutionPlan, snap StateSnapshot, opts WorkOptions) (ExecuteResult, error) {
	...
	for _, step := range plan.Steps {
		...
		if stepMutatesFeatureState(step.Command) {
			fs = e.refreshFeatureState(plan.Intent.Feature, fs)
		}
	}
	return res, nil
}

func stepMutatesFeatureState(command string) bool {
	switch command {
	case "plan", "enrich", "dev", "verify", "review":
		return true
	default:
		return false
	}
...
func (e *Executor) refreshFeatureState(feature string, current FeatureState) FeatureState {
	if e.Store == nil || strings.TrimSpace(e.RepoRoot) == "" {
		return current
	}
	return RefreshFeatureTaskState(e.RepoRoot, e.Config, e.Store, feature, current)
}
