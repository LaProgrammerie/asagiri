...
// PrintContinueFromRecommendation renders continue UX from a gate-aware recommendation.
func PrintContinueFromRecommendation(w io.Writer, rec NextRecommendation) {
	_, _ = fmt.Fprintln(w, "Continuing last active feature")
	_, _ = fmt.Fprintln(w, strings.Repeat("─", 30))
	_, _ = fmt.Fprintf(w, "Feature: %s\n", rec.Feature)
	if rec.TaskID != "" {
	...
	if strings.TrimSpace(rec.Reason) != "" {
		_, _ = fmt.Fprintln(w, rec.Reason)
	}
	_, _ = fmt.Fprintf(w, "Next:    %s\n", strings.TrimPrefix(rec.Primitive, "asa "))
	if rec.Action != "" && rec.Primitive != "" {
		_, _ = fmt.Fprintln(w, "Running:")
		_, _ = fmt.Fprintln(w, rec.Primitive)
	}
}
