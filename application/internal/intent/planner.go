...
func (p *DefaultPlanner) BuildPlan(ctx context.Context, intent ResolvedIntent, snap StateSnapshot, cfg *config.Config, opts WorkOptions) (ExecutionPlan, error) {
	...
	default:
		...
		add("dev", append(withTask(feat, taskID), "--agent", agent), "task_ready_for_dev", "implement task")
		add("dev", append(withTask(feat, taskID), "--agent", agent), "gate_needs_resume", "resume pending work gate")
...
// FeatureStateFor returns feature state from a snapshot (exported for CLI).
func FeatureStateFor(snap StateSnapshot, name string) FeatureState {
	return featureState(snap, name)
}
...
func EvaluateCondition(cond string, intent ResolvedIntent, fs FeatureState, opts WorkOptions) bool {
	...
	case "task_ready_for_dev":
		switch fs.NextTaskStatus {
		case asagiri.StatusEnriched, asagiri.StatusPending, asagiri.StatusPlanned, "":
			return !fs.EnrichGateBlocksDev
		default:
			return false
		}
	case "implementation_done":
		if hasBlockingPendingGate(fs) {
			return false
		}
		return fs.NextTaskStatus == asagiri.StatusImplemented || fs.NextTaskStatus == asagiri.StatusRunning
	case "gate_needs_resume":
		return pendingGateNeedsResume(fs)
	case "gate_blocking":
		return hasBlockingPendingGate(fs)
	case "verification_failed":
		return fs.NextTaskStatus == asagiri.StatusVerifyFailed
	case "review_enabled":
		return !opts.NoReview && !fs.VerifyEvidenceGateBlocksReview && !fs.TrustGateBlocksReview
...
func hasBlockingPendingGate(fs FeatureState) bool {
	return fs.PendingGate != nil && fs.PendingGate.Blocking
}

func pendingGateNeedsResume(fs FeatureState) bool {
	if !hasBlockingPendingGate(fs) {
		return false
	}
	return fs.PendingGate.Phase == gates.PendingPhaseResume
}

func recommendForPendingGate(fs FeatureState, feature, taskID string) (NextRecommendation, bool) {
	if !hasBlockingPendingGate(fs) {
		return NextRecommendation{}, false
	}
	pg := *fs.PendingGate
	reason := gates.FormatPendingAction(pg, feature)
	var primitive string
	switch pg.Phase {
	case gates.PendingPhaseSubmit:
		primitive = pg.SubmitCommand()
	case gates.PendingPhaseResume:
		primitive = pg.ResumeCommand()
	default:
		primitive = pg.SubmitCommand()
	}
