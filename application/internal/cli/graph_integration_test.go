...
func TestCLIIntegrationGraphCommands(t *testing.T) {
	...
	var planResult graphcli.PlanGraphResult
	...
	require.NoError(t, root.Execute(), jsonOut.String())
	var runResult graphcli.GraphRunJSONResult
	...
	require.NoError(t, root.Execute(), jsonOut.String())
	var resumeResult graphcli.GraphResumeResult
	...
	require.NoError(t, root.Execute(), jsonOut.String())
	var statusResult graphcli.GraphStatusResult
...
func TestCLIIntegrationGraphFlowRequired(t *testing.T) {
	...
	for _, args := range [][]string{
		...
		t.Run(strings.Join(args, "_"), func(t *testing.T) {
			...
			err := root.Execute()
			require.Error(t, err)
			require.ErrorIs(t, err, graphcli.ErrFlowRequired)
		})
	}
}
...
`)
	...
	for _, args := range [][]string{
		...
		t.Run(strings.Join(args, "_"), func(t *testing.T) {
			...
			err := root.Execute()
			require.Error(t, err)
			require.ErrorIs(t, err, graphcli.ErrNotEnabled)
		})
	}
}
...
func TestCLIIntegrationGraphResumeHonorsPersistedStrategy(t *testing.T) {
	...
	opts := graphcli.GraphRunOptionsFromPersisted(loaded)
...
func TestCLIIntegrationGraphCIFailsOnBudget(t *testing.T) {
	...
	err = root.Execute()
	require.Error(t, err)
	require.ErrorIs(t, err, graphcli.ErrCIFailed)
}
