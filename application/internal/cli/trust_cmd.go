...
func newVerifyTrustCmd() *cobra.Command {
...
// runTrustVerify is a test hook for integration tests.
func runTrustVerify(ctx context.Context, repoRoot string, cfg *config.Config, req trust.VerificationRequest) (trust.VerificationResult, error) {
	eng, closeStore := trustEngine(repoRoot, cfg)
	if closeStore != nil {
		defer closeStore()
	}
