...
func newNextCmd(dryRun *bool) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !noTrust && strings.TrimSpace(rec.TaskID) != "" {
				printDailyTaskTrust(cmd.OutOrStdout(), ctx, rec.TaskID)
			}
			if !execute {
				return nil
			}
	...
	cmd.Flags().BoolVar(&noTrust, "no-trust", false, "Masquer le résumé trust")
	return cmd
}
