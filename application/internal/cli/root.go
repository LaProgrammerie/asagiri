...
func newSpecCmd(dryRun *bool) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			defer ctx.Close()
			agent := workcli.ResolveWorkAgent(cmd, agentName, (*config.Config).WorkSpecAgent, ctx.Config)
...
func newEnrichCmd(dryRun *bool) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			defer ctx.Close()
			agent := workcli.ResolveWorkAgent(cmd, agentName, (*config.Config).WorkEnricherAgent, ctx.Config)
...
func newDevCmd(dryRun *bool) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			defer ctx.Close()
			agent := workcli.ResolveWorkAgent(cmd, agentName, (*config.Config).WorkDevAgent, ctx.Config)
...
func newVerifyCmd(dryRun *bool) *cobra.Command {
	...
	cmd.AddCommand(trustcli.VerifyCommand())
	return cmd
}
...
func newReviewCmd(dryRun *bool) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			defer ctx.Close()
			agent := workcli.ResolveWorkAgent(cmd, agentName, (*config.Config).WorkReviewerAgent, ctx.Config)
...
func newStatusCmd(dryRun *bool) *cobra.Command {
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !noTrust {
				...
				if err == nil {
					...
					if feature == "" && len(runs) > 0 {
						feature = runs[0].Feature
					}
					if feature != "" {
						_, _ = fmt.Fprintln(cmd.OutOrStdout())
						printDailyFeatureTrust(cmd.OutOrStdout(), ctx, feature)
					}
				}
			}
			return nil
	...
	cmd.Flags().BoolVar(&noTrust, "no-trust", false, "Masquer la section trust")
	return cmd
}
