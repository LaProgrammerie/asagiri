...
func RunDoctorChecks(repoRoot string, cfg *config.Config, opts DoctorOpts) []Check {
	...
	checks = append(checks, checkAgentRegistry(repoRoot)...)
...
func checkAgentRegistry(repoRoot string) []Check {
	dir := filepath.Join(repoRoot, agentspec.RegistryDir)
	...
	if err != nil {
		if os.IsNotExist(err) {
			return []Check{{
				ID:      "agents.registry",
				Status:  StatusWarn,
				Message: "Registry AgentSpec absent (.asagiri/agents/) — templates embarqués actifs",
				FixCLI:  "asa agents sync --write",
			}}
		}
		return []Check{{
			ID:      "agents.registry",
			Status:  StatusWarn,
			Message: fmt.Sprintf("Registry AgentSpec illisible: %v", err),
			FixCLI:  "asa agents sync --write",
		}}
	}
	count := 0
	...
	if count == 0 {
		return []Check{{
			ID:      "agents.registry",
			Status:  StatusWarn,
			Message: "Registry AgentSpec vide — templates embarqués actifs",
			FixCLI:  "asa agents sync --write",
		}}
	}
	return []Check{{ID: "agents.registry", Status: StatusOK, Message: fmt.Sprintf("%d AgentSpec(s) sur disque", count)}}
}
