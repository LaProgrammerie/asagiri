...
// RootCommand returns the `asa work` command.
...
Commandes expertes (asa prototype, flows, contracts, spec generate-from-product) restent optionnelles.`,
  ...
  asa next --feature crm-artisans`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			actx, err := opts.LoadWorkContext(startDir, *dryRun)
			if err != nil {
				return err
			}
			defer actx.Close()
			...
			layerOut, layerErr := HandleProductLayer(cmd.OutOrStdout(), instruction, scope, resolved, workOpts, actx.RepoRoot)
			if layerErr != nil {
				return layerErr
			}
			...
			plan, err := planner.BuildPlan(cmd.Context(), resolved, snap, actx.Config, workOpts)
			if err != nil {
				return err
			}
			...
			if actx.Config.Intent.DefaultMode == "guided" && !workOpts.Yes && !workOpts.PlanOnly && !estimateOnly && actx.Config.Work.RequirePlanConfirmation {
				if err := requireConfirm(workOpts, "Proceed with execution plan?"); err != nil {
					return err
				}
			}
			...
			if err != nil {
				...
				if errors.As(err, &pc) && !yes {
					...
					if confirmErr := requireConfirm(workOpts, pc.Error()); confirmErr != nil {
						return confirmErr
					}
			...
			if !workOpts.DryRun && !planOnly && !estimateOnly && len(v3res.Exec.Executed) > 0 {
				taskID := v3res.Exec.Intent.TaskID
				if taskID == "" {
					taskID = v3res.Estimate.TaskID
				}
				printPostWorkTrustLine(cmd.OutOrStdout(), actx, resolved.Feature, taskID, v3res.Exec.LastRunID)
			}

			if strictTrust && !workOpts.DryRun {
				flow, product, err := ResolveStrictTrust(actx.RepoRoot, trustFlow, resolved.Feature)
				if err != nil {
					return err
				}
...
func runInvestigationOnFailure(cmd *cobra.Command, actx *WorkContext, instruction string, depth investigation.Depth, noCloud bool) {
