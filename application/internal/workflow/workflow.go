...
type Service struct {
	...
	enrichGateAgentHook         gateAgentHook
	verifyEvidenceGateAgentHook gateAgentHook
}
...
func NewService(repoRoot string, cfg *config.Config, store *sqlite.Store, dryRun bool) *Service {
	if cfg != nil {
		...
		config.NormalizeWorkAgentObservability(&cfg.Work.AgentObservability)
	}
...
func (s *Service) EnrichFeature(ctx context.Context, feature, taskID, agentName string, force bool) (string, error) {
	...
	for _, task := range tasks {
		...
		if st := normalizeStatus(task.Status); st == asagiri.StatusEnriched && !force {
		...
		legacyEnrichPrompt := "Enrich task " + task.ID + " pour feature " + feature
		enrichPrompt, resolveErr := s.resolveAgentPrompt(agentresolve.PhaseEnrich, agentName, feature, task.ID, run.ID, legacyEnrichPrompt, contextFiles)
		if resolveErr != nil {
			_ = s.updateStep(run, "enrich", sqlite.StatusFailed, resolveErr.Error())
			return run.ID, resolveErr
		}
		...
		obs := agentobservability.New(s.repoRoot, task.ID, agentName, s.cfg)
		...
		candidateBody, err := json.Marshal(enriched)
		if err != nil {
			_ = s.updateStep(run, "enrich", sqlite.StatusFailed, err.Error())
			return run.ID, err
		}
		candidateJSON := string(candidateBody)
		if err := s.processEnrichGate(ctx, feature, task, candidateJSON, contextFiles); err != nil {
			_ = s.updateStep(run, "enrich", sqlite.StatusFailed, err.Error())
			return run.ID, err
		}
		finalPayload := candidateJSON
		if s.cfg != nil && s.cfg.Work.Gates.Enrich.IsActive() {
			if fresh, getErr := s.store.GetTask(task.ID); getErr == nil {
				task = *fresh
			}
			merged, mergeErr := mergeEnrichPayloadAfterGate(task.PayloadJSON, enriched)
			if mergeErr != nil {
				_ = s.updateStep(run, "enrich", sqlite.StatusFailed, mergeErr.Error())
				return run.ID, mergeErr
			}
			finalPayload = merged
		}
		if err := s.transitionTask(task, asagiri.StatusEnriched, force); err != nil {
...
func (s *Service) VerifyFeature(ctx context.Context, feature, taskID string, force bool) (string, error) {
	...
	for _, task := range tasks {
		...
		results, err := s.runVerification(ctx, targetDir, task.PayloadJSON)
		_ = s.persistValidationEvidence(task.ID, task.WorktreePath, results)
		if err != nil {
			_ = s.transitionTask(task, asagiri.StatusVerifyFailed, true)
			_ = s.updateStep(run, "verify", sqlite.StatusFailed, err.Error())
			return run.ID, err
		}
		if err := s.processVerifyEvidenceGate(ctx, feature, task, results); err != nil {
			msg := fmt.Sprintf("%s; re-run: asa verify %s --task %s --force", err.Error(), feature, task.ID)
			_ = s.updateStep(run, "verify", sqlite.StatusFailed, msg)
			return run.ID, fmt.Errorf("%s", msg)
		}
		if fresh, getErr := s.store.GetTask(task.ID); getErr == nil {
			task = *fresh
		}
		if err := s.processTrustGate(ctx, feature, task); err != nil {
			_ = s.updateStep(run, "verify", sqlite.StatusFailed, err.Error())
			return run.ID, err
		}
		if fresh, getErr := s.store.GetTask(task.ID); getErr == nil {
...
func (s *Service) ReviewFeature(ctx context.Context, feature, taskID, agentName string, force bool) (string, error) {
	...
	for _, task := range tasks {
		if err := s.workflowStepBlockedError(feature, task, "review"); err != nil {
			_ = s.updateStep(run, "review", sqlite.StatusFailed, err.Error())
			return run.ID, err
		}
		legacyReviewPrompt := "Review indépendante pour task " + task.ID
		reviewPrompt, resolveErr := s.resolveAgentPrompt(agentresolve.PhaseReview, agentName, feature, task.ID, run.ID, legacyReviewPrompt, nil)
		if resolveErr != nil {
			_ = s.updateStep(run, "review", sqlite.StatusFailed, resolveErr.Error())
			return run.ID, resolveErr
		}
