...
func (s *Service) processPlanGate(ctx context.Context, run *sqlite.Run, feature string, doc *spec.Document, canonicalTasks []asagiri.Task) error {
	...
	result, agentStdout, runErr := s.runPlanGate(ctx, run, feature, doc, canonicalTasks)
	if runErr != nil {
		result = gates.Result{
			GateID:   "plan_gate",
			GateType: "plan_gate",
...
func (s *Service) runPlanGate(ctx context.Context, run *sqlite.Run, feature string, doc *spec.Document, canonicalTasks []asagiri.Task) (gates.Result, string, error) {
	...
	legacyPrompt, err := buildPlanGatePrompt(doc, canonicalTasks)
	if err != nil {
		return gates.Result{}, "", err
	}
	agentKey := s.cfg.PlanGateAgent()
	runID := ""
	scopeID := feature
	if run != nil {
		runID = run.ID
	...
	prompt, err := s.resolveGatePrompt(agentresolve.PhasePlanGate, agentKey, feature, scopeID, runID, legacyPrompt, nil)
	if err != nil {
		return gates.Result{}, "", err
	}

	stdout, err := s.executeGateAgent(ctx, agentKey, feature, "", s.repoRoot, prompt, s.planGateAgentHook)
	if err != nil {
