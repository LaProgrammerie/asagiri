...
Implementation lives in `application/internal/cost/token_counter.go` and `estimator.go`. The estimator exists so Asagiri can show **order-of-magnitude** token and cost figures before it shells out to agents—useful for comparing against budgets and for logging what a plan roughly implied, without pretending to be a hosted provider's billing API.
