...
Before Asagiri forwards repository context to a cloud agent, it **investigates the project locally**. The implementation lives in `application/internal/investigation` and feeds both `asa investigate` and the V3 pipeline prelude. The engineering goal is narrow but important: **shrink prompt size and surface the files that matter** using bounded, repeatable tools—grep, filesystem scans, heuristics—instead of asking a model to ingest the whole tree.
