...
Asagiri follows idiomatic Go/Cobra exit semantics so shell scripts and CI gates can branch on `$?` without custom wrappers.
...
```bash
asa work                    # exit 2 — missing required instruction arg
asa doctor                  # exit 0 when all checks pass
asa work "x" --estimate-only # exit 0 when estimate succeeds
```
