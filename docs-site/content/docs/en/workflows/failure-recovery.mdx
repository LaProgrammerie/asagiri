...
`asa resume <run-id> --execute` chains steps only with global **`--dry-run`**. Non-dry-run resume does not auto-invoke agents — run the printed step manually or use `continue`.
</Callout>
