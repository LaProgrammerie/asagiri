...
Estimates multiply heuristic token counts by the rates in your `pricing` sheet. Provider invoices can diverge from that model, especially when vendors change tiers or bill by factors Asagiri does not model.
