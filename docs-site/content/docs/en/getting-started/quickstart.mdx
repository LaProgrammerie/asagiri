...
Install Asagiri (Homebrew, [install script](/docs/getting-started/installation#install-script-macos--linux), or `make build` from source), then from your project root:
...
`doctor` surfaces configuration gaps (missing providers on PATH, unknown logical agents, MCP settings, budget files) before you spend tokens on a full run.
