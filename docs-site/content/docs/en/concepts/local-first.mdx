...
Asagiri treats **local investigation and context reduction** as a first-class prelude. Before the expensive steps, Asagiri can narrow what leaves the repo:

1. **`asa investigate <feature>`** — bounded grep, candidate files, large-file warnings, related tests
2. **`asa context <feature> --optimize`** — collect, score, compress context into a pack
