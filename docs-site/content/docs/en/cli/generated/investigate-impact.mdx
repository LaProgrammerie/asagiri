---
title: Investigate Impact · Asagiri CLI
description: Analyser l'impact d'un changement produit
---

CLI path: **`asa investigate impact`**
...
> Original CLI synopsis: Analyser l'impact d'un changement produit

## Usage

```bash
asa investigate impact [flags]
```
...
| --- | --- | --- | --- |
| `--change` | Local | `(empty)` | Description du changement |
| `--dry-run` | Inherited | `false` | Simuler les exécutions sans lancer d'agent ni commandes externes |
| `--flow` | Local | `(empty)` | Flow concerné |
| `--product` | Local | `workspace-saas` | Produit |

## Examples

```bash
asa investigate impact
```
...
- [Investigate Graph](./investigate-graph.mdx)
