---
title: Agents Runs · Asagiri CLI
description: Historique des exécutions agents (ledger JSONL)
---

CLI path: **`asa agents runs`**
...
> Original CLI synopsis: Historique des exécutions agents (ledger JSONL)

## Usage

```bash
asa agents runs [flags]
```
...
| `--task` | Local | `(empty)` | Filtrer par task_id |
