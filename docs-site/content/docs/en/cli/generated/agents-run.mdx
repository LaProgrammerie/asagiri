---
title: Agents Run · Asagiri CLI
description: Inspecter un run agent (ledger + artefacts logs)
---

CLI path: **`asa agents run`**
...
> Original CLI synopsis: Inspecter un run agent (ledger + artefacts logs)

## Usage

```bash
asa agents run [flags]
```
...
| `--include-prompt` | Local | `false` | Inclure le contenu complet de prompt.md (avec --preview) |
| `--json` | Local | `false` | Sortie JSON sur stdout |
| `--preview` | Local | `false` | Aperçu replay read-only (artefacts + contenu JSON) |

## Examples

```bash
asa agents run run-abc
