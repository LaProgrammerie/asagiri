---
title: Prototype Create · Asagiri CLI
description: Créer un prototype local déterministe
---

CLI path: **`asa prototype create`**
...
> Original CLI synopsis: Créer un prototype local déterministe

## Usage

```bash
asa prototype create [flags]
```
...
| `--product` | Local | `(empty)` | Slug produit (sinon dérivé de l'intention) |
| `--stack` | Local | `react` | Stack prototype |
| `--style` | Local | `minimal` | Style prototype |
