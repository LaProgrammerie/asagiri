---
title: Session Create · Asagiri CLI
description: Créer une session
---

CLI path: **`asa session create`**
...
> Original CLI synopsis: Créer une session

## Usage

```bash
asa session create [flags]
```
...
| `--flow` | Local | `(empty)` | Flow lié |
| `--product` | Local | `(empty)` | Produit lié |
