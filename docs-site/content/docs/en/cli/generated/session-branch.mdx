---
title: Session Branch · Asagiri CLI
description: Créer une branche exploratoire
---

CLI path: **`asa session branch`**
...
> Original CLI synopsis: Créer une branche exploratoire

## Usage

```bash
asa session branch [flags]
```
...
| `--name` | Local | `(empty)` | Nom de la branche |
| `--type` | Local | `flow` | Type de branche |
...
- [Session Attach](./session-attach.mdx)
- [Session Create](./session-create.mdx)
- [Session Graph](./session-graph.mdx)
- [Session List](./session-list.mdx)
