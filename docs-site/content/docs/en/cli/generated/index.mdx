---
title: Index · Asagiri CLI
description: Indexe le dépôt dans .asagiri/index/chunks.sqlite (recherche LIKE).
---

CLI path: **`asa index`**

## When to use

Indexing keeps embedded search artefacts aligned whenever significant repository reorganisation threatens stale lookup tables.

> Original CLI synopsis: Indexer le dépôt pour le RAG local par mots-clés (spec §10.3)

## Usage

```bash
asa index [flags]
```

## Options

| Flag | Scope | Default | Description |
| --- | --- | --- | --- |
| `--dry-run` | Inherited | `false` | Simuler les exécutions sans lancer d'agent ni commandes externes |

## Examples

```bash
asa index
  asa index --dry-run
```

## What happens internally

- Crawls repositories to refresh Asagiri ingestion indexes referenced by tooling that depends on deterministic filesystem snapshots.

## Failure modes

- Cobra rejects unknown flags or arity mismatches before any repository side effects occur.
- Asagiri cannot resolve scaffolding when the command is invoked outside an initialised checkout.
- Delegated subprocesses (**git**, MCP hosts, editor agents, etc.) can exit non-zero and bubble errors verbatim.

## Related commands

- [Analysis](./analysis.mdx)
- [Architecture](./architecture.mdx)
- [Clean](./clean.mdx)
- [Completion](./completion.mdx)
- [Context](./context.mdx)
- [Continue](./continue.mdx)
- [Contracts](./contracts.mdx)
- [Cost](./cost.mdx)
- [Daemon](./daemon.mdx)
- [Dev](./dev.mdx)
- [Docs](./docs.mdx)
- [Doctor](./doctor.mdx)
- [Enrich](./enrich.mdx)
- [Estimate](./estimate.mdx)
- [Flows](./flows.mdx)
- [Graph](./graph.mdx)
- [Help](./help.mdx)
- [Impact](./impact.mdx)
- [Inbox](./inbox.mdx)
- [Init](./init.mdx)
- [Inspect](./inspect.mdx)
- [Investigate](./investigate.mdx)
- [Knowledge](./knowledge.mdx)
- [Mcp](./mcp.mdx)
- [Memory](./memory.mdx)
- [Next](./next.mdx)
- [Plan](./plan.mdx)
- [Pr](./pr.mdx)
- [Product](./product.mdx)
- [Prototype](./prototype.mdx)
- [Replay](./replay.mdx)
- [Report](./report.mdx)
- [Resume](./resume.mdx)
- [Review](./review.mdx)
- [Runtime](./runtime.mdx)
- [Session](./session.mdx)
- [Skills](./skills.mdx)
- [Spec](./spec.mdx)
- [Status](./status.mdx)
- [Sync](./sync.mdx)
- [Trust](./trust.mdx)
- [Verify](./verify.mdx)
- [Version](./version.mdx)
- [Work](./work.mdx)
