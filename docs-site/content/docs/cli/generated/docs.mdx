---
title: Docs · AgentFlow CLI
description: Documentation tooling for AgentFlow artefacts
---

CLI path: **`agentflow docs`**

## When to use

Use **docs** commands when refreshing machine-generated reference material—for example, syncing this MDX corpus with shipping CLI behaviour ahead of docs-site releases.

**generate-cli** is safe to run repeatedly; outputs are regenerated wholesale under the **--output** directory.

> Original CLI synopsis: Documentation tooling for AgentFlow artefacts

## Usage

```bash
agentflow docs [flags]
```

## Options

| Flag | Scope | Default | Description |
| --- | --- | --- | --- |
| `--dry-run` | Inherited | `false` | Simuler les exécutions sans lancer d'agent ni commandes externes |

## Examples

```bash
agentflow docs
```

> Wire richer samples through **cobra.Command.Example** when the happy path warrants copy/paste-friendly automation beyond this scaffold.

## What happens internally

- Holds documentation-adjacent automation that should remain orthogonal to primary workflow commands (does not mutate SQLite artefacts).

## Failure modes

- Cobra rejects unknown flags or arity mismatches before any repository side effects occur.
- AgentFlow cannot resolve scaffolding when the command is invoked outside an initialised checkout.
- Filesystem ACLs can deny writes under **--output**, leaving partial artefacts if interrupted mid-write.

## Related commands

- [Clean](./clean.mdx)
- [Completion](./completion.mdx)
- [Context](./context.mdx)
- [Continue](./continue.mdx)
- [Cost](./cost.mdx)
- [Dev](./dev.mdx)
- [Doctor](./doctor.mdx)
- [Enrich](./enrich.mdx)
- [Estimate](./estimate.mdx)
- [Help](./help.mdx)
- [Inbox](./inbox.mdx)
- [Index](./index.mdx)
- [Init](./init.mdx)
- [Inspect](./inspect.mdx)
- [Investigate](./investigate.mdx)
- [Mcp](./mcp.mdx)
- [Next](./next.mdx)
- [Plan](./plan.mdx)
- [Pr](./pr.mdx)
- [Report](./report.mdx)
- [Resume](./resume.mdx)
- [Review](./review.mdx)
- [Spec](./spec.mdx)
- [Status](./status.mdx)
- [Sync](./sync.mdx)
- [Verify](./verify.mdx)
- [Version](./version.mdx)
- [Work](./work.mdx)
