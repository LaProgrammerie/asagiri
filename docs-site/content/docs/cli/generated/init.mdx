---
title: Init · AgentFlow CLI
description: Crée .agentflow/, copie config.yaml si absent, et initialise state.sqlite.
---

CLI path: **`agentflow init`**

## When to use

Bootstrap commands scaffold or reconcile the AgentFlow working tree so subsequent workflow nodes can reliably resolve configuration and repository metadata.

> Original CLI synopsis: Initialiser AgentFlow dans le dépôt courant

## Usage

```bash
agentflow init [flags]
```

## Options

| Flag | Scope | Default | Description |
| --- | --- | --- | --- |
| `--dry-run` | Inherited | `false` | Simuler les exécutions sans lancer d'agent ni commandes externes |

## Examples

```bash
agentflow init
```

> Wire richer samples through **cobra.Command.Example** when the happy path warrants copy/paste-friendly automation beyond this scaffold.

## What happens internally

- Writes AgentFlow scaffolding (directories plus manifests) that align repositories with onboarding expectations documented for contributors.

## Failure modes

- Cobra rejects unknown flags or arity mismatches before any repository side effects occur.
- AgentFlow cannot resolve scaffolding when the command is invoked outside an initialised checkout.
- Downstream automation may fail whenever credentials, network endpoints, or cached prerequisites drift from expectations.

## Related commands

- [Clean](./clean.mdx)
- [Completion](./completion.mdx)
- [Context](./context.mdx)
- [Continue](./continue.mdx)
- [Cost](./cost.mdx)
- [Dev](./dev.mdx)
- [Docs](./docs.mdx)
- [Doctor](./doctor.mdx)
- [Enrich](./enrich.mdx)
- [Estimate](./estimate.mdx)
- [Help](./help.mdx)
- [Inbox](./inbox.mdx)
- [Index](./index.mdx)
- [Inspect](./inspect.mdx)
- [Investigate](./investigate.mdx)
- [Mcp](./mcp.mdx)
- [Next](./next.mdx)
- [Plan](./plan.mdx)
- [Pr](./pr.mdx)
- [Report](./report.mdx)
- [Resume](./resume.mdx)
- [Review](./review.mdx)
- [Spec](./spec.mdx)
- [Status](./status.mdx)
- [Sync](./sync.mdx)
- [Verify](./verify.mdx)
- [Version](./version.mdx)
- [Work](./work.mdx)
