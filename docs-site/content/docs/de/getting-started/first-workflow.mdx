...
Ohne `--execute` gibt **`resume` nur den nächsten Schritt aus** — manuell ausführen oder `continue` nutzen. Mit **`asa resume <run-id> --execute`** werden verbleibende Steps verkettet bis Ende, Gate oder `--max-steps` (echte Agenten ohne globales `--dry-run`).

Siehe [resume CLI](/docs/de/cli/resume) und [Fehlerbehebung](/docs/de/workflows/failure-recovery).
