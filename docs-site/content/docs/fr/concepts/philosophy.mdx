...
Asagiri existe pour rendre l’orchestration explicite. Vous configurez le pipeline, les commandes de validation et les garde-fous économiques ; l’outil coordonne l’exécution et enregistre ce qui a tourné.

## Comment Asagiri s’y prend

Le modèle mental par défaut est **spec → plan → implémentation → vérification → revue → rapport**. Chaque phase a une place définie dans la machine à états et dans les rapports sous `.asagiri/runs/`.
...
**La validation par vos commandes** signifie `go test`, les linters et les scripts en lesquels vous avez confiance — pas l’affirmation de l’agent qu’« tout va bien ». Asagiri planifie ces contrôles ; il ne remplace pas votre suite de tests.
...
Asagiri ne garantit ni correction, ni sécurité, ni préparation à la production. Il coordonne des outils ; les humains et votre suite de tests restent l’autorité.
</Callout>
...
Ces compromis sont intentionnels. Asagiri privilégie le **contrôle et l’auditabilité** à l’illusion d’un codage entièrement autonome.
