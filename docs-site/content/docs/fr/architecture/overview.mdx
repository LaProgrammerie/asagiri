...
Asagiri est une CLI Go (`application/cmd/asa`) dont la majeure partie du comportement vit dans `application/internal/` et les contrats partagés dans `application/pkg/asa`. Cette séparation garde le binaire fin tout en isolant parsing, planification, investigation, coût et stockage derrière des packages explicites que vous pouvez raisonner depuis la seule arborescence.
...
Runs et tâches persistent dans **SQLite** à `state.path` (défaut `.asagiri/state.sqlite`). Les artefacts de chaque run vont sous `.asagiri/runs/<run-id>/`, ce qui garde prompts, logs et sorties intermédiaires adressables sans relire la base.

## Points d'extension

Nouveaux agents via la config seule ; nouvelles portes qualité via `validation.commands` ; stratégies de routage sous `routing.strategies` ; outils MCP optionnels quand `mcp.enabled: true`. Ce sont des coutures intentionnelles : la plupart des équipes étendent Asagiri sans forker le point d'entrée Go.
