...
`asa resume <run-id> --execute` enchaîne sous global **`dry-run`** seulement — hors celui-ci aucune relance automatique agents : exécutez manuellement imprimées ou passez via `continue`.
</Callout>
