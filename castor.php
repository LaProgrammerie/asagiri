...
function npm_in_builder_volume(string $command): string
{
    return 'mkdir -p /var/www/application/node_modules && chown -R "$(id -u):$(id -g)" /var/www/application/node_modules || true; ' . $command;
}
...
function install(): void
{
    ...
    if (is_file("{$basePath}/yarn.lock")) {
        ...
        if (is_file("{$basePath}/package-lock.json")) {
            docker_compose_run(npm_in_builder_volume('npm ci'));
        } else {
            docker_compose_run(npm_in_builder_volume('npm install'));
        }
    }
