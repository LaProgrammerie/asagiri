...
| **Experience Platform UI (spec-ui, livré)** | [`spec-ui.md`](archives/specs/spec-ui.md), `docs/ai/06-spec-ui.md`, ADR-027 ; `application/internal/ui/` + `application/internal/cli/*_ui*.go` ; docs-site `en|fr|de|es` : `experience/index`, `mission-control`, `dashboard`, `command-palette`, `keyboard-shortcuts`, `mouse-support`, `themes`, `accessibility` |
| **Project Onboarding & Readiness (spec-onboarding, actif)** | [`spec-onboarding.md`](archives/specs/spec-onboarding.md), `docs/ai/06-spec-onboarding.md`, `docs/ai/active/handoff.md`, ADR-028 ; `.kiro/specs/project-onboarding/` ; code cible `application/internal/onboarding/` ; CLI `asa onboard`, `asa ready`, `asa doctor --full` |
