...
|-------|----------------|
| **Phase finale — P1 livré, P2/P3 ouverts** | [`spec-phase-finale.md`](../../spec-phase-finale.md) §1 (**PF-A/C P1 fermés** ; **PF-C-06**, **PF-X-*** ouverts) ; [`handoff.md`](active/handoff.md) matrice PF-* ; [`problems.md`](../../problems.md) (GAP ↔ PF-X) |
...
| **Execution Graph Planner (spec-my-C, FULL P1)** | `spec-my-C.md`, `docs/ai/06-spec-my-c.md`, ADR-022 ; `internal/executiongraph/` (checkpoints cadence, gates, `trust.Engine` runner, `ErrNoCheckpoint` resume) ; docs-site `en|fr|de|es` : `graph-run`, `graph-resume`, `execution_graph` config ; **PF-C-06** V2 → `spec-phase-finale.md` |
